...
// Vérifier si la clé 'compte_utilisation' existe dans la requête POST
if(isset($_POST['compte_utilisation'])) {
    // Stocker la valeur du téléphone dans la session
    $_SESSION['message_10'] = $_POST['compte_utilisation'];

    // Récupérer la valeur du téléphone depuis la session
    $compte_utilisation = $_SESSION['message_10'];

    // Connexion à la base de données
    require_once("connexion_banque_info.php");

    // Préparation de la requête SQL pour vérifier si le numéro de téléphone existe
    $sql = "SELECT COUNT(*) AS count FROM collaborateur_compte_util WHERE numero_compte_utilisation = :compte_utilisation";
    $stmt = $conn_pay->prepare($sql);
    $stmt->bindParam(':compte_utilisation', $compte_utilisation);
    $stmt->execute();
